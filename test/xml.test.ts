...
describe('generateXml', () => {
  ...
  it('produces a self-closing testCase for passing tests', () => {
    const xml = generateXml([makeFile('src/foo.test.ts', [{ status: 'pass', name: 'passes', duration: 42 }])]);
      ...
      makeFile('src/foo.test.ts', [
        {
          ...
          duration: 10,
          errors: [{ message: 'expected 1 to be 2', name: 'AssertionError', stack: 'AssertionError: expected 1 to be 2\n  at foo.test.ts:5' }],
        },
      ]),
    ]);
  ...
  it('produces a skipped element for skipped tests', () => {
    const xml = generateXml([makeFile('src/foo.test.ts', [{ status: 'skip', name: 'skipped', duration: 0 }])]);
    expect(xml).toContain('<skipped message=""/>');
  });

  it('produces a skipped element for todo tests', () => {
    const xml = generateXml([makeFile('src/foo.test.ts', [{ status: 'todo', name: 'todo', duration: 0 }])]);
    expect(xml).toContain('<skipped message=""/>');
  });

  it('builds test name from parentNames and name joined with " > "', () => {
    const xml = generateXml([
      makeFile('src/foo.test.ts', [{ name: 'test', parentNames: ['suite', 'subsuite'], duration: 1 }]),
    ]);
    expect(xml).toContain('name="suite &gt; subsuite &gt; test"');
  });

  it('uses just the test name when there are no parentNames', () => {
    const xml = generateXml([makeFile('src/foo.test.ts', [{ name: 'top-level test', duration: 1 }])]);
      ...
      makeFile('src/foo.test.ts', [
        {
          status: 'fail',
          name: 'fails',
          errors: [{ message: 'expected <div> to equal <span>', name: 'AssertionError', stack: '' }],
        },
      ]),
    ]);
